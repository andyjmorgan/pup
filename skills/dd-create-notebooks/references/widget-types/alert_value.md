...
Use for the current value or status of one monitor.
...
- Alert widgets require a valid `alert_id` (string) referencing an existing monitor.
  `alert_graph` also requires `viz_type` ("timeseries" or "toplist").
...
```bash
pup widgets schema alert_value --surface notebooks --data-source SOURCE --section request
```
