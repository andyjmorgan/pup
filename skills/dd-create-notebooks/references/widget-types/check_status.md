...
- Check status requires `check` (check name string), `grouping` ("cluster" or "check"), and `group_by` (tags to group by).
...
```bash
pup widgets schema check_status --surface notebooks --data-source SOURCE --section request
```
